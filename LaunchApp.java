...
final public class LaunchApp extends IO{
    ...
    static void exit() {
        println();
        println("@ THANK YOU FOR YOUR VIST @ :-) Good Bye...");
        println();
        System.exit(0);
    }
    // list all exams - upcomming and past exams
    static void examList() {
        ...
        println();
        println("________________UPCOMMING EXAMS___________________________________________");
        Exams.getInstance().listAll(true, Today.today);
        println();
    }
    // To add new Exam with date and details
    static void addExam() throws IOException, InterruptedException {
        ...
        try {
            ...
            if(instance.isPresent(date)) {
                print("\nTHERE IS A EXAM ON THIS DATE ! \nPlease select option 2 from main menu to edit.");
            }
            else {
            ...
            print("Press enter to continue...");    System.in.read();
            main();
        }
    }
